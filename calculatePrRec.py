import pathlib
from main import compare  # (path1, path2, printGraph=plotPath)
import csv

dataDir = pathlib.Path('/home/alexsun8/streamlabs/radio/data/Список радио сравнения трансляций')
resFile = pathlib.Path('/home/alexsun8/streamlabs/radio/data/PrRes.txt')
...
def iterate(data_dir, printRes=True):
    ...
    for folder in data_dir.glob('*'):
        ...
        for fileBase in folder.glob(r'ЦРВ_*.ts'):
            file1 = fileBase
        for fileNew in folder.glob(r'АРВ_*.ts'):
            file2 = fileNew
        ...
        if file1 and file2 and printRes:
            ...
            istheSame, inclTime = compare(str(file1), str(file2), printGraph=f'./testPlots2/{str(folder.stem)}.png')
            if istheSame:

...
from hashes.dejavu.dejavu.config.settings import (CONNECTIVITY_MASK, DEFAULT_AMP_MIN,
...
from hashes.pyhash import fnv1a_32
        ...
        for j in range(1, fan_value):
            if (i + j) < len(peaks):
                ...
                if MIN_HASH_TIME_DELTA <= t_delta <= MAX_HASH_TIME_DELTA:
                    ...
                    # h = hex(fnvhash.fnv1a_32(f"{str(freq1)}|{str(freq2)}|{str(t_delta)}".encode('utf-8')))
                    # fp = pyhash.farm_fingerprint_64
                    hasher = fnv1a_32
                    s = f"{str(freq1)}|{str(freq2)}|{str(t_delta)}".encode('utf-8')
                    h = hasher(s) # todo choose hashFunc

                    # hashes.append((h.hexdigest()[0:FINGERPRINT_REDUCTION], t1))
                    hashes.append((h[0:FINGERPRINT_REDUCTION], t1))
